...
import { MessageSquare, Plus, Edit, Trash2, Copy, Sparkles } from "lucide-react"
import { mockTalkingPoints, type TalkingPoint } from "@/lib/mock-talking-points"

interface CountryTalkingPointsProps {
  countryCode: string
  countryName: string
}

export default function CountryTalkingPoints({ countryCode, countryName }: CountryTalkingPointsProps) {
  ...
  // Filter talking points for this country
  const countryTalkingPoints = mockTalkingPoints.filter((tp) => tp.country === countryCode)

  // Get unique categories for filter
  const categories = Array.from(new Set(countryTalkingPoints.map((tp) => tp.category)))

  // Filter by category if needed
  const filteredTalkingPoints =
    filter === "all" ? countryTalkingPoints : countryTalkingPoints.filter((tp) => tp.category === filter)
      ...
      <div className="flex justify-between items-center">
        <div>
          <h2 className="text-2xl font-bold flex items-center">
            <MessageSquare className="mr-2 h-5 w-5" /> Talking Points for {countryName}
          </h2>
          <p className="text-muted-foreground mt-1">
            Manage talking points and discussion topics for meetings and briefings
          </p>
        </div>
          ...
          <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
            ...
            <DialogContent className="sm:max-w-[600px]">
              <DialogHeader>
                ...
                <DialogDescription>
                  Add talking points for {countryName} that can be used in reports and briefings.
                </DialogDescription>
              </DialogHeader>
        ...
        <Card>
          <CardContent className="pt-6 text-center py-10">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              {filter === "all"
                ? `No talking points have been added for ${countryName} yet.`
                : `No talking points found in the ${filter} category.`}
            </p>
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="sm:max-w-[600px]">
          <DialogHeader>
            ...
            <DialogDescription>Update talking points for {countryName}.</DialogDescription>
          </DialogHeader>
